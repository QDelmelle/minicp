...
public class SMoney {
    ...
    public static void main(String[] args) {
        ...
        notEqual(values[S.val], 0);
        notEqual(values[M.val], 0);
        cp.post(equal(values[M.val], carry[3]));
        equal(sum(carry[2], values[S.val], values[M.val], minus(values[O.val]), mul(carry[3], -10)), 0);
        equal(sum(carry[1], values[E.val], values[O.val], minus(values[N.val]), mul(carry[2], -10)), 0);
        equal(sum(carry[0], values[N.val], values[R.val], minus(values[E.val]), mul(carry[1], -10)), 0);
        equal(sum(values[D.val], values[E.val], minus(values[Y.val]), mul(carry[0], -10)), 0);
