...
public class Eternity {
    ...
    public static void main(String[] args) {
        ...
        for(int i=0; i< pieces.length; i++){
            table[i*4][0] = table[i*4+1][0] = table[i*4+2][0] = table[i*4+3][0] = i;
            table[i*4][1] = table[i*4+1][2] = table[i*4+2][3] = table[i*4+3][4] = pieces[i][0];
            table[i*4][2] = table[i*4+1][3] = table[i*4+2][4] = table[i*4+3][1] = pieces[i][1];
            table[i*4][3] = table[i*4+1][4] = table[i*4+2][1] = table[i*4+3][2] = pieces[i][2];
            table[i*4][4] = table[i*4+1][1] = table[i*4+2][2] = table[i*4+3][3] = pieces[i][3];
        }
        ...
        for(int i=0; i<n; i++){
            for (int j=0; j<m;j++){
                IntVar[] x = {id[i][j], u[i][j], r[i][j], d[i][j], l[i][j]};
                cp.post(new TableCT(x, table));
            }
        }

        // Constraint3: place "0" on all external sides of the border (gray color)
        for(int i=0; i<n; i++){
            l[i][0].assign(0);
            r[i][m-1].assign(0);
        }
