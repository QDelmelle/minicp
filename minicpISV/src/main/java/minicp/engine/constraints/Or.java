...
public class Or extends AbstractConstraint { // x1 or x2 or ... xn
    ...
    public void propagate() {
        ...
        boolean wlBound = true;
        boolean wrBound = true;
        while(wlBound || wrBound) {

            if (x[wL.value()].isBound()) {
                if (x[wL.value()].isTrue()) {
                    setActive(false);
                    break;
                } else wL.setValue(wL.value() + 1);
            } else {
                wlBound = false;
                x[wL.value()].propagateOnBind(this);
            }

            if (x[wR.value()].isBound()) {
                if (x[wR.value()].isTrue()) {
                    setActive(false);
                    break;
                } else wR.setValue(wR.value() - 1);
            } else {
                wrBound = false;
            ...
            if (wR.value() == wL.value()) {
                x[wR.value()].assign(true);
                break;
            }
        }
    }
